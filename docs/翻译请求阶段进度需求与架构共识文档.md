...
| 1.1 | 2026-08-15 | 纳入已批准的 `savingHistory` 第六阶段，与翻译历史需求对齐 |
...
翻译历史需求已批准在原五阶段后增加真实的保存阶段，当前定义如下：
...
| `savingHistory` | 正在保存翻译历史 | 已取得完整译文，正在提交本机翻译历史 |

### 6.1 不公开 `finalizing`

完成处理通常很短，不增加泛化的 `finalizing` 阶段。`savingHistory` 是已有的具体持久化操作，不等同于泛化完成处理；如果以后需要其他可感知阶段，必须作为独立需求修订本文。
...
stateDiagram-v2
    ...
    CheckingCache --> SavingHistory: L1/L2 命中
    WaitingForContent --> SavingHistory: 一次性完整结果
    ReceivingContent --> SavingHistory: 流式完整结束
    SavingHistory --> Success: 保存完成或按既有降级规则结束
    CheckingCache --> Error
